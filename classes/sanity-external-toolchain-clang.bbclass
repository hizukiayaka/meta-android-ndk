...
def check_toolchain_sanity(d, generate_events=False):
    ...
    if d.getVar('EXTERNAL_TARGET_SYS', True) == 'UNKNOWN':
        raise_exttc_sanity_error('Unable to locate prefixed clang binary for %s in EXTERNAL_TOOLCHAIN_BIN (%s)' % (d.getVar('TARGET_SYS', True), d.getVar('EXTERNAL_TOOLCHAIN_BIN', True)), d, generate_events)

    # Test 3: clang binary exists
    clang = d.expand('${EXTERNAL_TOOLCHAIN_BIN}/${EXTERNAL_TARGET_SYS}-clang')
    if not os.path.exists(clang):
        raise_exttc_sanity_error('Compiler path `%s` does not exist' % clang, d, generate_events)
